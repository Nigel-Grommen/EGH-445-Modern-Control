...
x1_lin     = (ylin(:,1) + x_bar(1))/m;    % X position [physical, m]
x2_lin     = ylin(:,2);               % X velocity 
x3_lin     = (ylin(:,3) + x_bar(3))/m;    % Y position [physical, m]
...
ylim([-1, 4]);   % physical X position range
...
clear functions
t_cl       = 0:Ts:120;                                         % Simulation time vector [0 to 80s in steps of Ts]
...
% Performance Metrics 
printPerformanceMetrics(t_sim0, x_sim0/m, t_sim, x_sim/m, x_bar/m, 'Full-State', 1, 3)
...
subplot(3,2,1);
plot(t_sim0, x_sim0(:,1)/m, 'b', t_sim, x_sim(:,1)/m, 'g--');
ylim([0, 5]);    % physical X position range
yline(x_bar(1)/m, 'r--');
...
subplot(3,2,3);
plot(t_sim0, x_sim0(:,3)/m, 'b', t_sim, x_sim(:,3)/m, 'g--');
yline(x_bar(3)/m, 'r--');
...
x1_lin = (x_lin_cl(:,1) + x_bar(1))/m;   % X position [physical, m]
x2_lin = x_lin_cl(:,2);              % X velocity
x3_lin = (x_lin_cl(:,3) + x_bar(3))/m;   % Y position [physical, m]
...
plot(t_sim0, x_sim0(:,1)/m,      '-', 'Color', col_nl,  'LineWidth', lw);
plot(t_obs0, y_obs0(:,1)/m,      '-', 'Color', col_obs, 'LineWidth', lw);
ylim([0, 5]);    % physical X position range
yline(x_bar(1)/m, 'k:', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('X Position (m)');
title('Horizontal Position');
...
plot(t_sim0, x_sim0(:,3)/m,      '-', 'Color', col_nl,  'LineWidth', lw);
plot(t_obs0, y_obs0(:,2)/m,      '-', 'Color', col_obs, 'LineWidth', lw);
yline(x_bar(3)/m, 'k:', 'LineWidth', 1.2);
ylim([0 3.5])
xlabel('Time (s)'); ylabel('Y Position (m)');
title('Altitude');
legend('L+IC', 'NL+IC', 'NL+OBS+IC', 'Target', 'Location', 'southeast');
...
subplot(3,2,1);
plot(t_obs0, y_obs0(:,1)/m,      'b',  'LineWidth', lw); hold on
plot(t_obs,  y_obs(:,1)/m,       'g--','LineWidth', lw);
yline(x_bar(1)/m, 'r--', 'LineWidth', 1.2);
ylim([0 5]);
xlabel('Time (s)'); ylabel('X Position (m)');
title('Horizontal Position');
legend('No Disturbance', 'Disturbance', 'Target', 'Location', 'southeast');
...
legend('No Disturbance', 'Disturbance');
subplot(3,2,3);
plot(t_obs0, y_obs0(:,2)/m,      'b',  'LineWidth', lw); hold on
plot(t_obs,  y_obs(:,2)/m,       'g--','LineWidth', lw);
ylim([0 3.5])
yline(x_bar(3)/m, 'r--', 'LineWidth', 1.2);
xlabel('Time (s)'); ylabel('Y Position (m)');
title('Altitude');
legend('No Disturbance', 'Disturbance', 'Target', 'Location', 'southeast');
subplot(3,2,4);
...
printPerformanceMetrics(t_obs0, y_obs0/m, t_obs, y_obs/m, x_bar/m, 'Observer', 1, 2)
...
yline(-40, 'r--', 'LineWidth', 1.2);
ylabel('Force (N)'); title('F1 — Lateral Force');
legend('No Disturbance', 'Disturbance');
...
yline(0,  'r--', 'LineWidth', 1.2);
ylabel('Force (N)'); title('F2 — Thrust');
legend('No Disturbance', 'Disturbance');
xlim([0 40]); ylim([-5 85]);
...
plot(t_obs0, y_obs0(:,3)*180/pi, 'b', 'LineWidth', lw); hold on
plot(t_obs,  y_obs(:,3)*180/pi,  'g--', 'LineWidth', lw);
ylabel('Angle (deg)'); title('Roll Angle — Tilt');
legend('No Disturbance', 'Disturbance');
xlabel('Time (s)');
xlim([0 40]);
...
for i = 1:length(test_IDs)
    for j = 1:size(x0_tests,1)
        ...
        plot(results_t{i,j}, results_y{i,j}(:,1)/m, ...
             line_styles{j}, 'Color', colours(i,:), ...
             'LineWidth', 1.5, 'DisplayName', label);
    end
end
yline(x_bar(1)/m, 'r--', 'LineWidth', 1.2, 'DisplayName', 'Target');
xlabel('Time (s)'); ylabel('X Position (m)');
title('Horizontal Position');
legend('Location', 'southeast', 'FontSize', 7);
ylim([-2 6]);    % physical X position range

subplot(2,1,2); hold on;
for i = 1:length(test_IDs)
    for j = 1:size(x0_tests,1)
        plot(results_t{i,j}, results_y{i,j}(:,2)/m, ...
             line_styles{j}, 'Color', colours(i,:), ...
             'LineWidth', 1.5, 'HandleVisibility', 'off');
    end
end
yline(x_bar(3)/m, 'r--', 'LineWidth', 1.2, 'DisplayName', 'Target');
xlabel('Time (s)'); ylabel('Y Position (m)');
title('Altitude');
...
subplot(3,1,1);
plot(t_cl, x_real_log(:,1)/m,  'm',   'LineWidth', 2.0); hold on
plot(t_cl, x_lue_log(:,1)/m,   'r--', 'LineWidth', 1.0);
plot(t_cl, x_kf_log(:,1)/m,    'b--', 'LineWidth', 1.5);
xlabel('Time (s)'); ylabel('X Position (m)');
title('X Position'); ylim([0 5]);   % physical
legend('Real', 'Luenberger', 'Kalman Filter', 'Location', 'southeast');

subplot(3,1,2);
plot(t_cl, x_real_log(:,3)/m,  'm',   'LineWidth', 2.0); hold on
plot(t_cl, x_lue_log(:,3)/m,   'r--', 'LineWidth', 1.0);
plot(t_cl, x_kf_log(:,3)/m,    'b--', 'LineWidth', 1.5);
xlabel('Time (s)'); ylabel('Y Position (m)');
title('Y Position');
legend('Real', 'Luenberger', 'Kalman Filter', 'Location', 'southeast');
